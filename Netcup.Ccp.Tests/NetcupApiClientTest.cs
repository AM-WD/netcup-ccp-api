	...
	public class NetcupApiClientTest
	{
		...
		public async Task ShouldGetInfoDnsZone(string clientRequestId)
		{
			...
			var res = new ResponseMessage<DnsZone>
			{
				...
				StatusCode = 2000,
				Status = ResponseStatus.Success,
				ResponseData = new DnsZone(ttl: 86400, refresh: 28800, retry: 7200, expire: 1209600, dnssecstatus: false)
				{
					Name = "netcup.de",
					Serial = 2024060199,
				}
		...
		public async Task ShouldGetInfoDnsRecords(string clientRequestId)
		{
			...
			var res = new ResponseMessage<DnsRecordSet>
			{
				...
				ResponseData = new DnsRecordSet
				{
					Records =
					[
						new DnsRecord("@", DnsRecordType.A, "46.38.224.30")
						{
							Id = 123,
						},
						new DnsRecord("@", DnsRecordType.Aaaa, "2a03:4000::e01e")
						{
							Id = 456,
						}
					]
			...
			Assert.AreEqual(DnsRecordType.A, response.ResponseData.Records[0].Type);
			...
			Assert.AreEqual(DnsRecordType.Aaaa, response.ResponseData.Records[1].Type);
		...
		public async Task ShouldUpdateDnsZone(string clientRequestId)
		{
			...
			var res = new ResponseMessage<DnsZone>
			{
				...
				StatusCode = 2000,
				Status = ResponseStatus.Success,
				ResponseData = new DnsZone(ttl: 86400, refresh: 28800, retry: 7200, expire: 1209600, dnssecstatus: true)
				{
					Name = "netcup.de",
					Serial = 2024060199,
				}
		...
		{
			// Arrange
			var zone = new DnsZone(ttl: 1, refresh: 1, retry: 1, expire: 0, dnssecstatus: false);
			var client = GetClient();
		...
		{
			// Arrange
			var zone = new DnsZone(ttl: 1, refresh: 0, retry: 1, expire: 1, dnssecstatus: false);
			var client = GetClient();
		...
		{
			// Arrange
			var zone = new DnsZone(ttl: 1, refresh: 1, retry: 0, expire: 1, dnssecstatus: false);
			var client = GetClient();
		...
		{
			// Arrange
			var zone = new DnsZone(ttl: 0, refresh: 1, retry: 1, expire: 1, dnssecstatus: false);
			var client = GetClient();
		...
		[DataTestMethod]
		[DataRow(null, DnsRecordType.A, null, null, null)]
		[DataRow(111u, DnsRecordType.Aaaa, null, null, null)]
		[DataRow(null, DnsRecordType.Mx, 10u, null, null)]
		[DataRow(222u, DnsRecordType.A, null, true, null)]
		[DataRow(null, DnsRecordType.Ns, null, null, "ClientRequestId")]
		public async Task ShouldUpdateDnsRecords(uint? id, DnsRecordType type, uint? prio, bool? delete, string clientRequestId)
		{
			// Arrange
			var res = new ResponseMessage<DnsRecordSet>
			{
			...
			var request = new DnsRecordSet
			{
				Records =
				[
					new DnsRecord("@", type, "46.38.224.30")
					{
						Id = id,
						Priority = prio,
						DeleteRecord = delete,
			...
			Assert.AreEqual(GetEnumMemberValue(type), req["param"]["dnsrecordset"]["dnsrecords"][0].Value<string>("type"));
		...
		public async Task ShouldThrowArgumentNullForHostnameOnUpdateDnsRecords(string hostname)
		{
			...
			var request = new DnsRecordSet
			{
				Records =
				[
					new DnsRecord(hostname, DnsRecordType.A, "46.38.224.30")
					{
						Id = null,
					}
				]
		...
		public async Task ShouldThrowArgumentNullForPriorityOnUpdateDnsRecords()
		{
			...
			var request = new DnsRecordSet
			{
				Records =
				[
					new DnsRecord("@", DnsRecordType.Mx, "46.38.224.30")
					{
						Id = null,
						Priority = null,
					}
		...
		public async Task ShouldThrowArgumentNullForDestinationOnUpdateDnsRecords(string destination)
		{
			...
			var request = new DnsRecordSet
			{
				Records =
				[
					new DnsRecord("@", DnsRecordType.A, destination)
					{
						Id = null,
					}
				]
		...
		private string GetEnumMemberValue(Enum value)
		{
			...
			var attr = fi.GetCustomAttributes(typeof(EnumMemberAttribute), inherit: false)
